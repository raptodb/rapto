...
- **Scalability**: The runtime uses a completely single-threaded event-driven architecture based exclusively on `epoll`. This linear model avoids synchronization, cache contention, and overhead introduced by threads or context-switching. In addition, memory remains preallocated to maintain predictable and stable behavior even under high workloads.
